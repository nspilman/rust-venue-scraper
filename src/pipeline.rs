...
use crate::types::{EventApi, RawDataInfo, EventArgs};
...
impl Pipeline {
    ...
    fn process_event(api: &dyn EventApi, raw_event: &serde_json::Value) -> Result<Option<ProcessedEvent>> {
    ...
    fn persist_to_json(events: &[ProcessedEvent], api_name: &str, output_dir: &str) -> Result<String> {
        // Ensure output directory exists
        fs::create_dir_all(output_dir)?;
        
        // Generate filename with timestamp
        let timestamp = Utc::now().format("%Y%m%d_%H%M%S");
        let filename = format!("{}_{}.json", api_name, timestamp);
