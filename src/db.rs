...
impl DatabaseManager {
    ...
    pub async fn delete_venue_data(&self, venue_slug: &str) -> Result<()> {
        ...
        info!("Deleting venue data for '{}'", venue_slug);
        
        // Simple approach: delete by venue slug/name in JSON data
        let venue_result = conn.execute(
            "DELETE FROM nodes WHERE label = 'venue' AND (
                json_extract(data, '$.slug') = ? OR 
                json_extract(data, '$.name') = ? OR
                json_extract(data, '$.venue_name') = ?
            )",
            libsql::params![venue_slug, venue_slug, venue_slug]
        ).await.map_err(|e| ScraperError::Database {
            message: format!("Failed to delete venue nodes: {e}")
        })?;
        
        info!("Deleted {} venue records", venue_result);
        
        // Delete events for this venue
        let event_result = conn.execute(
            "DELETE FROM nodes WHERE label = 'event' AND (
                json_extract(data, '$.venue_slug') = ? OR
                json_extract(data, '$.venue_name') = ? OR
                json_extract(data, '$.venue') = ?
            )",
            libsql::params![venue_slug, venue_slug, venue_slug]
        ).await.map_err(|e| ScraperError::Database {
            message: format!("Failed to delete event nodes: {e}")
        })?;
        
        info!("Deleted {} event records", event_result);
        
        // Delete raw data for this venue's API
        let api_name = format!("crawler_{}", venue_slug);
        let raw_result = conn.execute(
            "DELETE FROM nodes WHERE label = 'raw_data' AND json_extract(data, '$.api_name') = ?",
            libsql::params![api_name]
        ).await.map_err(|e| ScraperError::Database {
            message: format!("Failed to delete raw data nodes: {e}")
        })?;
        
        info!("Deleted {} raw data records", raw_result);
        ...
        let edge_result = conn.execute(
            ...
            message: format!("Failed to delete orphaned edges: {e}")
        })?;
        
        info!("Deleted {} orphaned edges", edge_result);
        
        let total_deleted = venue_result + event_result + raw_result;
        info!("Successfully deleted {} total records for venue '{}'", total_deleted, venue_slug);
